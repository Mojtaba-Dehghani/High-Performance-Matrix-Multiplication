...
void matrix_mult_unrolling_cache_friendly(int n, double* a, double* b, double* c) {
    double* B_T = (double*)_aligned_malloc(n * n * sizeof(double), 64);
    register int i, j, k;
    ...
    int unroll_limit = (n / 6) * 6;

    for (int ii = 0; ii < unroll_limit; ii += BS) {
        int i_end = MIN(ii + BS, unroll_limit);

        for (int jj = 0; jj < unroll_limit; jj += BS) {
            int j_end = MIN(jj + BS, unroll_limit);

            for (i = ii; i < i_end; i += 6) {
                for (j = jj; j < j_end; j += 6) {
    ...
    for (i = 0; i < unroll_limit; i++) {
        for (j = unroll_limit; j < n; j++) {
            register double cij = 0;
            register double *at = a + i * n;
            register double *bt = B_T + j * n;
            for (k = 0; k < n; k++) cij += (*at++) * (*bt++);
...
#define MIN(a, b) (((a) < (b)) ? (a) : (b))
typedef double uv4df __attribute__((vector_size(32), aligned(1)));
typedef double v4df __attribute__((vector_size(32)));

void matrix_mult_unrolling_cache_friendly_vec(int n, double* a, double* b, double* c) {
    for (register int i = 0; i < n * n; i++) {
        c[i] = 0.0;
    }

    register int BS = 64;
    register int unroll_limit_i = (n / 4) * 4;
    register int unroll_limit_j = (n / 8) * 8;

    for (register int ii = 0; ii < unroll_limit_i; ii += BS) {
        register int i_end = MIN(ii + BS, unroll_limit_i);

        for (register int kk = 0; kk < n; kk += BS) {
            register int k_end = MIN(kk + BS, n);

            for (register int jj = 0; jj < unroll_limit_j; jj += BS) {
                register int j_end = MIN(jj + BS, unroll_limit_j);

                for (register int i = ii; i < i_end; i += 4) {

                    for (register int j = jj; j < j_end; j += 8) {
                        register v4df c00 = *(uv4df*)&c[(i + 0) * n + j];
                        register v4df c01 = *(uv4df*)&c[(i + 0) * n + j + 4];
                        register v4df c10 = *(uv4df*)&c[(i + 1) * n + j];
                        register v4df c11 = *(uv4df*)&c[(i + 1) * n + j + 4];
                        register v4df c20 = *(uv4df*)&c[(i + 2) * n + j];
                        register v4df c21 = *(uv4df*)&c[(i + 2) * n + j + 4];
                        register v4df c30 = *(uv4df*)&c[(i + 3) * n + j];
                        register v4df c31 = *(uv4df*)&c[(i + 3) * n + j + 4];

                        register double *ap0 = &a[(i + 0) * n + kk];
                        register double *ap1 = &a[(i + 1) * n + kk];
                        register double *ap2 = &a[(i + 2) * n + kk];
                        register double *ap3 = &a[(i + 3) * n + kk];
                        register double *bp = &b[kk * n + j];

                        for (register int k = kk; k < k_end; k++) {
                            register v4df b_vec0 = *(uv4df*)(bp);
                            register v4df b_vec1 = *(uv4df*)(bp + 4);
                            bp += n;

                            register double a0 = *ap0++;
                            c00 += a0 * b_vec0;
                            c01 += a0 * b_vec1;

                            register double a1 = *ap1++;
                            c10 += a1 * b_vec0;
                            c11 += a1 * b_vec1;

                            register double a2 = *ap2++;
                            c20 += a2 * b_vec0;
                            c21 += a2 * b_vec1;

                            register double a3 = *ap3++;
                            c30 += a3 * b_vec0;
                            c31 += a3 * b_vec1;
                        }

                        *(uv4df*)&c[(i + 0) * n + j] = c00;
                        *(uv4df*)&c[(i + 0) * n + j + 4] = c01;

                        *(uv4df*)&c[(i + 1) * n + j] = c10;
                        *(uv4df*)&c[(i + 1) * n + j + 4] = c11;

                        *(uv4df*)&c[(i + 2) * n + j] = c20;
                        *(uv4df*)&c[(i + 2) * n + j + 4] = c21;

                        *(uv4df*)&c[(i + 3) * n + j] = c30;
                        *(uv4df*)&c[(i + 3) * n + j + 4] = c31;
                    }
                }
            }
        }
    ...
    for (register int i = 0; i < n; i++) {
        ...
        for (register int j = 0; j < n; j++) {
            ...
            if (i >= unroll_limit_i || j >= unroll_limit_j) {
                register double cij = c[i * n + j];
                register double *ap = &a[i * n];
                register double *bp = &b[j];
...
void matrix_mult_unrolling_cache_friendly_vec_omp(int n, double* a, double* b, double* c) {
    ...
    int BS = 64;
    int unroll_limit_i = (n / 4) * 4;
    int unroll_limit_j = (n / 8) * 8;

    #pragma omp parallel for
    for (int ii = 0; ii < unroll_limit_i; ii += BS) {
        register int i_end = MIN(ii + BS, unroll_limit_i);
...
int main() {
    ...
    do {
        ...
        fflush(stdin);
        printf("\n\nDo you want to run matrix_mult_unrolling_cache_friendly (y/n)? ");
        if (getchar() == 'y') {
            if (++ref > 2) ref = 2;
            t0 = clock();
            matrix_mult_unrolling_cache_friendly(n, A, B, ref == 1 ? C1 : C2);
            t1 = clock();
            printf("\n\t\t\tExecution time of matrix_mult_unrolling_cache_friendly = %0.2f s", (float)(t1 - t0) / CLOCKS_PER_SEC);
        }

        fflush(stdin);
        printf("\n\nDo you want to run matrix_mult_unrolling_cache_friendly_vec (y/n)? ");
        if (getchar() == 'y') {
            if (++ref > 2) ref = 2;
            t0 = clock();
            matrix_mult_unrolling_cache_friendly_vec(n, A, B, ref == 1 ? C1 : C2);
            t1 = clock();
            printf("\n\t\t\tExecution time of matrix_mult_unrolling_cache_friendly_vec = %0.2f s", (float)(t1 - t0) / CLOCKS_PER_SEC);
        }

        */

        fflush(stdin);
        printf("\n\nDo you want to run matrix_mult_unrolling_cache_friendly_vec_omp (y/n)? ");
        if (getchar() == 'y') {
            if (++ref > 2) ref = 2;

            /*
            t0 = clock();
            matrix_mult_unrolling_cache_friendly_vec_omp(n, A, B, ref == 1 ? C1 : C2);
            t1 = clock();
            printf("\n\t\t\tExecution CPU time = %0.2f s", (float)(t1 - t0) / CLOCKS_PER_SEC);
            */

            double start_time = omp_get_wtime();
            matrix_mult_unrolling_cache_friendly_vec_omp(n, A, B, ref == 1 ? C1 : C2);
            double end_time = omp_get_wtime();
            printf("\n\t\t\tExecution Wall-Clock time of matrix_mult_unrolling_cache_friendly_vec_omp = %0.4f s", end_time - start_time);
        }
